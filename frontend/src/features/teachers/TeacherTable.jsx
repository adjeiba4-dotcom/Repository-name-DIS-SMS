...
const COLUMNS = [
  { key: "name", label: "Teacher", sortable: true, className: "" },
  {
    key: "staffNo",
    label: "Staff No",
    sortable: true,
  ...
  { key: "department", label: "Department", sortable: true, className: "" },
  {
    key: "gender",
    label: "Gender",
    sortable: true,
...
export default function TeacherTable({
  ...
  const endIndex = Math.min(startIndex + pageSize, sortedTeachers.length);
  const pageRows = sortedTeachers.slice(startIndex, endIndex);
  ...
  return (
    <Panel
      title={
        title || (isArchived ? "Archived Teachers" : "Teacher Directory")
      }
      description={
        description ||
        (isArchived
          ? "Soft-deleted teacher records. Restore to return them to the active directory."
          : "Browse teacher records. Use actions to view, edit, or archive.")
      }
    >
      {isEmpty ? (
        <EmptyState
          icon={Users}
          title={isArchived ? "No archived teachers" : "No teachers found"}
          description={
            isArchived
              ? "Archived records will appear here after soft-delete."
              : "Try adjusting search or filters, or register a new teacher."
          }
        />
      ) : (
        <>
          ...
          <div className="mt-[var(--space-4)] flex flex-col gap-[var(--space-3)] border-t border-[var(--color-border-muted)] pt-[var(--space-4)] sm:flex-row sm:items-center sm:justify-between">
            <Caption variant="muted" size="sm" className="m-0">
              Showing {sortedTeachers.length === 0 ? 0 : startIndex + 1}–
              {endIndex} of {sortedTeachers.length}
            </Caption>
