import { BookUser, Eye, Pencil, RotateCcw, Trash2 } from "lucide-react";
...
/**
 * Active / archived teacher-subject directory powered by shared DataTable.
 */
export default function TeacherSubjectList({
  ...
  const filters =
    !isArchived && onStatusChange
      ? [
          {
            id: "teacher-subject-status-filter",
            label: "Status",
            value: status,
  ...
  return (
    <DataTable
      ...
      title={
        title ||
        (isArchived
          ? "Archived Teacher Subjects"
          : "Teacher Subject Directory")
      }
      description={
        description ||
        (isArchived
          ? "Soft-deleted assignments. Restore to return them to the active directory."
          : "Assign teachers to subjects by academic year, term, and weekly periods.")
      }
      columns={COLUMNS}
      rows={rows}
      loading={loading}
      ...
      emptyDescription={
        isArchived
          ...
          : "Try adjusting search or filters, or create a new assignment."
      }
      ...
      searchable
      search={search}
      searchLabel={isArchived ? "Search archived" : "Search assignments"}
      searchPlaceholder="Search by teacher, subject, year, or term…"
