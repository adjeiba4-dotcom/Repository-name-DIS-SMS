...
export default function ExaminationSummaries({
  scope = "overview",
  onScopeChange,
  stats = null,
  loading = false,
  ...
  return (
    <section className="space-y-[var(--space-4)]">
      ...
      <Panel
        title="Breakdown"
        description="Grouped examination and score metrics."
      >
        {loading ? (
          ...
          <Body variant="muted" size="sm" className="m-0">
            No examination data for this summary window.
          </Body>
        ) : (
            ...
            <table className="min-w-full border-collapse">
              <thead>
                <tr className="border-b border-[var(--color-border-muted)] text-left">
                  {["Group", "Examinations", "Scores", "Avg Marks"].map(
                    (heading) => (
                      <th
                        key={heading}
                    ...
                    <td className="px-[var(--space-3)] py-[var(--space-3)]">
                      <Body size="sm" className="m-0">
                        {scope === "type" || scope === "overview"
                          ? formatExaminationType(row.label)
                          : row.label}
                      </Body>
                    </td>
                    <td className="px-[var(--space-3)] py-[var(--space-3)]">
